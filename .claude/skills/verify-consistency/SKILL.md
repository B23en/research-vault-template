...
1. **Conflict** — a note contradicts a line in `Direction.md`, or two notes
   contradict each other. Report what each side rests on; do not rule on which
   one is wrong. `Direction.md` holds the current direction, and evidence can
   run against it — that judgment is the user's.
...
## Conflicts
<!-- Each: [[link]] the note, the Direction.md line or [[other note]] it runs
     against, and what each side rests on. Do not rule on which is wrong. -->
